        ...
        while self._keep_running:
            ...
            if len(command_packet) != 13:
                ...
                if K in [0x0F, 0x1F]:
                    ...
                    self._log.debug("-> AZ_DIVISOR:        " + hex(self._pulses_per_degree))
                    self._log.debug("-> EL_DIVISOR:        " + hex(self._pulses_per_degree))

                    self._ser.write(bytearray(response_packet))

                    self._log.debug(
                        "Response packet sent: " + str(list(map(hex, list(response_packet))))
                    )
                elif K == 0x2F:
